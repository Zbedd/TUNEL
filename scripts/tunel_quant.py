...
from tunel_quant import summarize, yolo_model_training
from tunel_quant import plotting

def main(cfg):
    '''PIPELINE FUNCTIONS'''
    # Optionally, train the model
    if cfg['seg_method'] == 'yolo':
        yolo_model_training.main()
    
    analysis = summarize.analyze_folder(cfg['input_folder'], cfg['seg_method'], cfg['conThresh'], cfg['kSize'])
    summary = summarize.summarize_analysis(analysis, cfg['location_map'])
    ...
    summary_plot = plotting.plot_summary(title = 'Percentage of cells alive by group (test data)', include_likely=True, plot_dots = False, plot_sample_size=True, include_location = True, include_other = False, flip_group_location=True, add_significance=True)
    
    '''WRITING FUNCTIONS'''
    date_str = datetime.now().strftime('%Y-%m-%d')
    output_path = os.path.join(cfg['output_folder'], f'summary_{date_str}.csv')
